...
const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`${isUser ? 'max-w-[80%]' : 'max-w-[95%]'} ${isUser ? 'order-2' : 'order-1'}`}>
        <div
        ...
        {/* Interactive Content */}
        {ic && (
          <Suspense fallback={<div className="text-xs text-white/40 py-2">Loading...</div>}>
            ...
            {ic.type === 'confetti' && (
              <ConfettiCelebration message={ic.confettiMessage || 'Great choice, Skipper!'} />
            )}
        ...
        {!message.failed && message.quickReplies && message.quickReplies.length > 0 && (
