...
const KitBagView: React.FC<{ open: boolean; onClose: () => void }> = ({ open, onClose }) => {
  const { apiClient, sessionId, cartItems, removeFromCartAt } = useChatContext();
  const [purchases, setPurchases] = useState<PurchaseItem[]>([]);
  const [cartProducts, setCartProducts] = useState<Array<{ id: number; name: string; price: number; image_url?: string; category?: string }>>([]);
  ...
  const idToProduct = React.useMemo(() => {
    const map: Record<number, { id: number; name: string; price: number; image_url?: string; category?: string }> = {};
  ...
  const cartTotal = React.useMemo(() => {
    return cartItems.reduce((sum, id) => sum + (idToProduct[id]?.price ?? 0), 0);
  ...
  useEffect(() => {
    ...
    if (profile) {
      ...
      if (profile.preferred_surface) info['Surface'] = profile.preferred_surface;
    ...
    if (sessionId) {
      setLoading(true);
      apiClient.getUserProfile(sessionId).then((data) => {
        if (data?.purchase_history) {
          setPurchases(
            (data.purchase_history as unknown as PurchaseItem[]).map((item) => ({
              product_name: item.product_name || (item as any).name || 'Unknown',
              price: item.price ?? 0,
  ...
  useEffect(() => {
    ...
    apiClient.getCartProducts(cartItems).then((products) => {
      setCartProducts(
        products.map((p) => ({
          id: p.id,
          name: p.name,
            ...
            <div className="p-4">
              ...
              {Object.keys(profileInfo).length > 0 && (
                <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-4">
                  <p className="text-[10px] text-white/40 uppercase tracking-wider mb-2 font-medium">Your Profile</p>
              ...
              <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-4">
                ...
                {cartProductsLoading && cartItems.length > 0 && (
                  <div className="text-xs text-white/40 py-2">Loading cart...</div>
                )}
                {!cartProductsLoading && cartItems.length > 0 && (
                  <div className="space-y-2 mt-2">
                    {cartItems.map((productId, index) => {
                      const item = idToProduct[productId];
                      const icon = item ? (CATEGORY_ICONS[item.category || 'bat'] || '\u{1F3CF}') : '\u{1F3CF}';
                      return (
                        <div key={`${productId}-${index}`} className="flex gap-2 items-center bg-white/5 rounded p-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-white font-medium truncate">{item?.name ?? `Product #${productId}`}</p>
                            <p className="text-[10px] text-white/40">NPR {item?.price?.toLocaleString() ?? '—'}</p>
                          </div>
                          <button
                            type="button"
                ...
                {!cartProductsLoading && cartItems.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-white/10 flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-white">NPR {cartTotal.toLocaleString()}</span>
                  </div>
                )}
                {!cartProductsLoading && cartItems.length > 0 && cartProducts.length === 0 && (
                  <p className="text-xs text-white/40 mt-2">Cart items could not be loaded.</p>
                )}
              </div>
              ...
              <div>
                ...
                {loading && (
                  <div className="text-xs text-white/40 py-4 text-center">Loading...</div>
                )}
                ...
                <div className="space-y-2">
                  {purchases.map((item, idx) => {
                    ...
                    const icon = CATEGORY_ICONS[item.category || 'bat'] || '\u{1F3CF}';
                        ...
                        <div className="flex gap-2 items-start">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-white/40">
                              {days} days ago &middot; NPR {item.price?.toLocaleString()}
                            </p>
                          </div>
                        </div>
                        {needsReGrip && (
                          <div className="mt-1.5 bg-amber-500/10 border border-amber-500/20 rounded px-2 py-1">
                            <p className="text-[10px] text-amber-400">
                              Time to re-grip your bat? Fresh grips improve control!
                            </p>
                          </div>
                        )}
                      </div>
