    ...
    try {
      ...
      if (result.products.length > 0) {
        addBotMessage(
          ...
          { productCards: result.products, quickReplies: ['Show me more', 'Different category', 'Talk to human'] }
        );
      } else {
        addBotMessage(
      ...
      >
        <div className="flex space-x-2">
          ...
          <button
            ...
            className="bg-[#262626] border border-white/10 rounded-button px-3 py-2.5 text-white/50 hover:text-white/80 hover:bg-[#333] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          ...
          <input
            ...
            type="text"
            autoComplete="off"
            placeholder={selectedFile ? 'Send to find matching products...' : 'Describe what you need, paste a link, or paste an image...'}
            value={inputText}
          ...
          <button
            type="submit"
            disabled={(!inputText.trim() && !selectedFile) || isTyping || uploading}
            className="bg-white hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed text-[#1A1A1A] px-5 py-2.5 rounded-button transition-colors focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[#1A1A1A] font-semibold"
        ...
        <p id="chat-help-text" className="text-xs text-white/50 mt-2">
          Press Enter to send {selectedFile ? '| Image attached' : '| Upload or paste an image to find similar products'}
        </p>
      </form>
    </div>
  );
